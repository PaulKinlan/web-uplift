...
| `a11ytree` | what assistive technology actually receives: the computed accessibility tree (roles, computed names, ignored subtrees with reasons, and the focusable/level/checked/... flags) - so an `aria-label` overridden by `aria-labelledby`, a name that comes from `title`, or a subtree hidden by an ancestor `aria-hidden` shows up here and NOT in a DOM-attribute probe - plus the REAL tab order walked with CDP key events, where each stop records the element, its on-screen state, its computed focus indicator, and whether it sits inside `aria-hidden` (which removes it from the a11y tree but NOT from the tab order). Judgement against `be-inclusive` names-and-roles and structure-and-focus. Capped by default at 400 projected nodes and 60 focus stops, with the caps echoed (tree.maxNodes, focusOrder.maxStops) whenever a sample is truncated; raise them with `--max-nodes <n>` and `--max-stops <n>` on a page bigger than that | Accessibility.getFullAXTree + Input.dispatchKeyEvent |
